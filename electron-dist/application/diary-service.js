...
import { loadDiaryEvents, loadDiarySummaries, localDateKeyFromTime, saveDiaryEvents, saveDiarySummaries, } from '../infrastructure/storage/diary-storage.js';
...
function absenceTitleFor(summary, templates, seed) {
    ...
    const variant = s % 3;
    if (variant === 0)
        return composeTitleParts([primary, moodSuffix], s);
    if (variant === 1)
        return composeTitleParts([primary, closingSuffix], s + 3);
...
function quietNormalTitleFor(summary, templates, seed) {
    ...
    const variant = s % 4;
    if (variant === 0)
        return composeTitleParts([lowTitle, quietMood], s);
    if (variant === 1)
        return composeTitleParts([tierTitle, softMood], s + 2);
    if (variant === 2)
        return composeTitleParts([quietMood, lowTitle], s + 5);
...
function aggregateDayEvents(dayEvents, fallbackTier) {
    const visitTier = dayEvents.find((event) => event.type === 'day_visit')?.payload?.relationshipTier ??
        fallbackTier;
...
function getCachedEntry(dateKey) {
...
async function finalizeCompletedDaysInner(dataRoot, _templates, relationshipTier) {
    const eventsData = await loadDiaryEvents(dataRoot);
