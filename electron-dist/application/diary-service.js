...
import { loadDiaryEntries, loadDiaryEvents, loadDiarySummaries, localDateKeyFromTime, saveDiaryEntries, saveDiaryEvents, saveDiarySummaries, } from '../infrastructure/storage/diary-storage.js';
...
function isQuietNormalDay(summary) {
...
function firstEncounterFromMeta(firstEncounterDate) {
...
/** 可浏览的最早日期：初遇日前一天（序幕） */
export function getEarliestDiaryDateKey(firstEncounterDate) {
    return addDaysToDateKey(firstEncounterDate, -1);
}
export async function loadDiaryTemplates(contentRoot) {
    const filePath = path.join(contentRoot, 'diary', 'templates.json');
    const raw = await readFile(filePath, 'utf8');
    return diaryTemplatesSchema.parse(JSON.parse(raw));
}
function aggregateDayEvents(dayEvents, fallbackTier) {
    const visitTier = dayEvents.find((event) => event.type === 'day_visit')?.payload?.relationshipTier ?? fallbackTier;
...
function buildSummary(dateKey, dayEventsByDate, fallbackTier, firstEncounterDate) {
...
function resolveSummaryForDate(dateKey, summariesByDate, dayEventsByDate, fallbackTier, firstEncounterDate) {
    return (summariesByDate[dateKey] ??
        buildSummary(dateKey, dayEventsByDate, fallbackTier, firstEncounterDate));
}
function buildDiaryEntryCacheKey(summary, finalized) {
    // AIGC START
...
async function resolveEntryForDate(params) {
    // AIGC START
    const { contentRoot, dateKey, today, relationshipTier, firstEncounter, dayEventsByDate, summariesByDate, entriesData, } = params;
    ...
    const summary = resolveSummaryForDate(dateKey, summariesByDate, dayEventsByDate, relationshipTier, firstEncounter);
...
function moodTagFor(summary, templates, seed) {
...
function highlightsFor(summary) {
    if (summary.narrativeKind === 'prelude_stranger') {
        return ['序幕', '还不认识'];
    }
    if (summary.narrativeKind === 'first_encounter') {
...
function pushActivityParagraphs(paragraphs, summary, templates, seed) {
    if (summary.interactionCount === 0) {
...
export function generateDiaryEntry(summary, templates) {
    ...
    if (summary.narrativeKind === 'first_encounter') {
        ...
        const hasActivity = summary.interactionCount > 0 ||
            summary.fedFoodLabels.length > 0 ||
            summary.focusStartedCount > 0 ||
            summary.focusCompletedCount > 0 ||
            summary.focusInterruptedCount > 0;
...
export async function recordDiaryDayVisit(dataRoot, relationshipTier) {
...
export async function appendDiaryEvent(dataRoot, partial) {
    const eventsData = await loadDiaryEvents(dataRoot);
...
async function finalizeCompletedDaysInner(dataRoot, templates, relationshipTier) {
    const eventsData = await loadDiaryEvents(dataRoot);
    ...
    let fallbackTier = relationshipTier;
    ...
    while (compareDateKeys(cursor, today) < 0) {
        ...
        fallbackTier = summary.relationshipTier;
    ...
    const compactedEvents = eventsData.events.filter((event) => compareDateKeys(event.dateKey, today) >= 0);
    const eventsDirty = compactedEvents.length !== eventsData.events.length ||
...
export async function runDiarySessionHooks(dataRoot, contentRoot, relationshipTier) {
...
function entryBeforeStoryRange(dateKey, firstEncounter) {
    return compareDateKeys(dateKey, getEarliestDiaryDateKey(firstEncounter)) < 0;
}
export async function getDiaryEntryForDate(dataRoot, contentRoot, dateKey, relationshipTier) {
    const today = localDateKeyFromTime(Date.now());
    const eventsData = await loadDiaryEvents(dataRoot);
    const entriesData = await loadDiaryEntries(dataRoot);
    const summariesData = await loadDiarySummaries(dataRoot);
...
export async function listRecentDiaryEntries(dataRoot, contentRoot, limit, relationshipTier) {
    const today = localDateKeyFromTime(Date.now());
    const eventsData = await loadDiaryEvents(dataRoot);
    const entriesData = await loadDiaryEntries(dataRoot);
    const summariesData = await loadDiarySummaries(dataRoot);
