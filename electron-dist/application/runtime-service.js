...
function normalizeFocusGoal(goal) {
    const trimmed = goal?.trim().replace(/\s+/g, ' ') ?? '';
    return trimmed ? trimmed.slice(0, 40) : null;
}
function clampFocusDurationMinutes(duration) {
    if (!Number.isFinite(duration)) {
        return 25;
    }
    return Math.min(240, Math.max(1, Math.round(duration)));
}
function focusReviewSuffix(reviewTone) {
    switch (reviewTone) {
        case 'done':
            return ' 这轮我给你好好记上了。';
        case 'partial':
...
export class RuntimeService {
    ...
    lastFocusHeartbeatPersistAt = 0;
    recentAmbientLineIds = [];
    state;
    dialogueSequence = 0;
    recentClickAt = [];
    recentFeedAt = [];
    ...
    dataRoot;
    appVersion;
    constructor(contentRoot, dataRoot, appVersion) {
    ...
    async triggerStartupGreeting() {
        ...
        const line = this.decorateSpeechLine(this.pickLineWithFallback(this.getTimeGreetingDialogueType(period), 'idle'));
        this.state = {
            ...this.state,
            mainState: 'idle',
    ...
    buildFocusContextLine() {
        return this.decorateSpeechLine(this.pickLineWithFallback('focus_during_interaction', 'focus_progress'));
    }
    applyFocusContextAffordance(mainState, line) {
        this.state = {
            ...this.state,
            mainState,
            currentExpression: line.expressionHint,
    ...
    async handlePetClick() {
        ...
        const priorInWindow = this.recentClickAt.filter((t) => nowMs - t <= CLICK_SPAM_WINDOW_MS).length;
        ...
        if (this.couldOfferBranchPlotNow(nowDate, wouldSpam, celebration)) {
            ...
            const choices = plot.choices.map((c) => ({
                id: c.id,
                label: c.label,
            }));
            this.state = {
    ...
    async resolveBranchPlotChoice(choiceId) {
    ...
    async requestComfort(tone) {
        ...
        const explicitType = tone === 'heavy' ? 'interaction_comfort_heavy' : 'interaction_comfort_light';
    ...
    async handleFeed(foodType) {
        const nowDate = new Date();
        const now = nowDate.toISOString();
        this.applySatietyDecayToState(nowDate);
        if (this.state.stats.satiety >= 100) {
            const line = this.decorateSpeechLine(this.pickLineWithFallback('feed_reject_full', 'interaction'));
            this.state = {
                ...this.state,
                mainState: 'feedback',
                currentExpression: line.expressionHint,
        ...
        if (!isFoodUnlockedForTier(foodItem, this.state.relationshipTier)) {
            const lockedText = this.formatSpeechText(`这份 ${foodItem.label} 我先记下啦，等我们到「${getFoodUnlockTierLabel(foodItem.unlockTier)}」再一起吃，好不好。`);
            this.state = {
                ...this.state,
                mainState: 'feedback',
        ...
        this.state = {
            ...
            lastFeedAt: now,
            relationshipTier: resolveRelationshipTier(this.state.stats.favorability + effect.favorability + Math.max(0, scoreBonus)),
            stats: {
                ...this.state.stats,
                satiety: clampStat(this.state.stats.satiety + feedSatietyGain),
                satietyClockAt: now,
                interactionCount: this.state.stats.interactionCount + 1,
                favorability: this.state.stats.favorability + effect.favorability + Math.max(0, scoreBonus),
    ...
    async debugShowDialogue(type, tierOverride) {
        ...
        if (tierOverride && ['low', 'mid', 'high'].includes(tierOverride)) {
            this.state = { ...this.state, relationshipTier: tierOverride };
        }
        const raw = this.pickLineWithFallback(type, 'interaction');
        const isFeedish = raw.type.startsWith('feed_food_') ||
            [
                'feed_sweet',
                'feed_fruit',
                'feed_drink',
    ...
    async debugShowBranchPlot(plotId) {
        const plot = this.getInteractivePlot(plotId);
        if (!plot) {
    ...
    async handlePetLongPress() {
        ...
        const line = this.decorateSpeechLine(this.pickLineWithFallback('interaction_long_press', 'interaction'));
        this.state = {
            ...this.state,
            mainState: resolveMainState('longPress'),
            currentExpression: line.expressionHint,
    ...
    async handlePetContextMenu() {
        ...
        const line = this.decorateSpeechLine(this.pickLineWithFallback('interaction_context_menu', 'interaction_long_press'));
        this.state = {
            ...this.state,
            mainState: resolveMainState('longPress'),
            currentExpression: line.expressionHint,
    ...
    async startFocusSession(duration, goal) {
    ...
    async focusHeartbeat() {
        ...
        const line = this.decorateSpeechLine(this.pickLineWithFallback('focus_progress', 'focus_start'));
        const now = new Date().toISOString();
        this.state = {
            ...this.state,
            mainState: resolveMainState('focusStart'),
    ...
    async completeFocusSession(reviewTone) {
        ...
        this.state = {
            ...
            stats: {
                ...
                lastFocusDurationMinutes: completedDurationMinutes > 0
    ...
    async updateWindowPosition(position) {
        this.state = {
            ...this.state,
    ...
    getWindowPositionForDisplayMode(mode) {
    ...
    async updateWindowPositionForDisplayMode(mode, position) {
        const slot = getWindowPositionSlotForDisplayMode(mode);
        this.state = {
            ...this.state,
            windowPosition: position,
    ...
    decorateSpeechLine(line, milestoneLabel) {
        return { ...line, text: this.formatSpeechText(line.text, milestoneLabel) };
    }
    applyActivitySatietyDrain(now, points) {
        if (points <= 0) {
            return;
        }
    ...
    shouldOfferHungerLines() {
        return this.state.stats.satiety <= SATIETY_HUNGER_THRESHOLD;
    }
    nextComfortStateAfterSpeech(reason, nowDate) {
        if (!reason) {
    ...
    nextTimeGreetingStateAfterSpeech(period, nowDate) {
        const dateKey = localDateKeyFromDate(nowDate);
        const base = this.state.timeGreetingState.dateKey === dateKey
    ...
    getTimeGreetingDialogueType(period) {
        switch (period) {
            case 'morning':
                return 'idle_morning';
            case 'noon':
    ...
    shouldOfferTimeGreeting(period, nowDate) {
        const dateKey = localDateKeyFromDate(nowDate);
        const state = this.state.timeGreetingState.dateKey === dateKey
            ? this.state.timeGreetingState
            : {
                dateKey,
                greetedPeriods: [],
    ...
    pickComfortLineIfNeeded(source, nowDate, forcedReason) {
        ...
        if (!forcedReason && source === 'idle' && this.state.comfortState.lastComfortAt) {
            const elapsed = nowDate.getTime() - new Date(this.state.comfortState.lastComfortAt).getTime();
            if (elapsed < COMFORT_IDLE_COOLDOWN_MS) {
                return null;
            }
        }
        return {
            line: this.pickLineWithFallback(source === 'idle' ? 'idle_comfort' : 'interaction_comfort', source === 'idle' ? 'idle' : source === 'double' ? 'interaction_double' : 'interaction'),
            comfortReason: reason,
        };
    }
    detectComfortReason(source, nowDate) {
        const nowMs = nowDate.getTime();
        const hour = nowDate.getHours();
        const isLateNight = hour >= 23 || hour < 5;
        if (this.state.comfortState.lastFocusInterruptAt) {
            const elapsed = nowMs - new Date(this.state.comfortState.lastFocusInterruptAt).getTime();
    ...
    celebrationMilestonePatch(celebration, year) {
        if (!celebration) {
            return {};
        }
    ...
    pickClickLine() {
        ...
        this.recentClickAt = this.recentClickAt.filter((timestamp) => now - timestamp <= CLICK_SPAM_WINDOW_MS);
    ...
    pickIdleLine() {
        ...
        if (this.shouldOfferTimeGreeting(timeGreetingPeriod, nowDate)) {
            return {
                line: this.pickLineWithFallback(this.getTimeGreetingDialogueType(timeGreetingPeriod), 'idle'),
                timeGreetingPeriod,
            };
        }
    ...
    pickFeedLine(foodType, foodCategory, shouldUseRepeatLine) {
        if (shouldUseRepeatLine) {
            return this.pickLine('feed_repeat');
        }
        return this.pickLineWithFallback(getFoodDialogueType(foodType), FOOD_EFFECTS[foodCategory].dialogueType);
    }
    pickLineWithFallback(primaryType, fallbackType) {
        try {
    ...
    pickLine(type) {
        ...
        return pickDialogue(this.contentBundle, type, this.nextDialogueSeed(), this.state.relationshipTier);
    }
    pickIdleAmbientLine(period) {
        ...
        const freshLines = dedupedLines.filter((line) => !this.recentAmbientLineIds.includes(line.id));
    ...
    getIdleAmbientCandidateTypes(period) {
        const seed = this.nextDialogueSeed();
        const candidateTypes = ['idle'];
        switch (period) {
            case 'morning':
                if (seed % 100 < 25) {
    ...
    getScopedLinesForType(type) {
        ...
        const exactTierLines = allLines.filter((line) => line.relationshipTier === this.state.relationshipTier);
    ...
    rememberRecentAmbientLine(lineId) {
    ...
    formatFoodLine(line, foodLabel, preferenceScore) {
        const preferenceSuffix = preferenceScore >= 5
            ? '，这口是真的很对胃口。'
            : preferenceScore === 4
                ? '，今天这份投喂我会好好记着。'
                : preferenceScore === 3
    ...
    resolveCompletedFocusDurationMinutes() {
...
function getWindowPositionSlotForDisplayMode(mode) {
    return mode === 'panel' ? 'panel' : 'desktop';
}
function removeNicknamePlaceholder(text) {
    return text
