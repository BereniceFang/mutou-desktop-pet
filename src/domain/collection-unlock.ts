// AIGC START
import type { AppState } from '../shared/content-schema.js'

export function isBadgeUnlocked(badgeId: string, state: AppState): boolean {
...
export function isStoryCardUnlocked(cardId: string, state: AppState): boolean {
  const { stats, relationshipTier } = state
  switch (cardId) {
    case 'story_01':
      return true
